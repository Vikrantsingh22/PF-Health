...
Supabase Google OAuth authentication and durable user-owned history are explicitly approved for Vercel deployment. The Google-verified email identity is the only accepted real personal datum and is managed by Supabase Auth; application tables reference the immutable Auth user UUID and do not duplicate email. PF Health does not request Google API access or read/persist provider tokens in application storage. The landing page remains public. Guided Ravi, Laboratory, and history require authentication. Users may list and delete only their own runs and sessions. This approval does not permit real PF identifiers, employers, member records, government credentials, analytics, or external corrections.
