...
The laboratory uses a separate strict `pf-health-synthetic-scenario` version 1 document. It contains one workflow and 1–6 employments with generated opaque IDs, generated `Synthetic Employer A–F` labels, one current employment, strict ISO dates, enum exit reasons, and account group `A|B|C`. Unknown fields are rejected. Session draft/snapshot versions, assessments, evidence graphs, actor plans, audit events, and runtime IDs are not part of exported scenario JSON.
