...
# Laboratory scenario model

The laboratory uses a separate strict `pf-health-synthetic-scenario` version 1 document. It contains one workflow and 1–6 employments with generated opaque IDs, required fictional employer labels limited to 2–60 characters, one current employment, strict ISO dates, enum exit reasons, and account group `A|B|C`. Presets and newly added records begin with generated `Synthetic Employer A–F` labels, which judges may replace with fictional labels; the interface explicitly rejects real employer data. Unknown fields are rejected. Session draft/snapshot versions, assessments, evidence graphs, actor plans, audit events, and runtime IDs are not part of exported scenario JSON.
