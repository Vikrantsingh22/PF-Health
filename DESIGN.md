...
The welcome surface introduces the Case File silhouette before Ravi is loaded. Its upper-left card corner is square beneath a deep-blue tab whose two sides taper inward in opposite directions before meeting a softly curved top. A deep-blue side tab protrudes from behind the right edge with a straight inner edge and one tapered outer lower edge that meets a thin rail directly. The rail continues to the card's square lower-right corner and rounds only its outer bottom-right corner. Together these layers read as one physical file silhouette rather than inset decoration.
